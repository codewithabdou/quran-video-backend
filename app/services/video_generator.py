...
def generate_video(request: VideoRequest) -> str:
    ...
    # Resolution usually refers to the shorter side for landscape (Example: 720p = 1280x720)
    # For portrait (Reel), it refers to the shorter side as well (width).
    # Common Ratios: 16:9 (YouTube) and 9:16 (Reel)
    ...
    # Ensure dimensions are divisible by 2 (required for many codecs and prevents some numpy errors)
    ...
    # If 720p (width 720), ratio is 1.0. If 360p (width 360), ratio is 0.5.
    ...
    ARABIC_FONT_SIZE = int(settings.FONT_SIZE * scale_ratio)
    ENGLISH_FONT_SIZE = int(settings.FONT_SIZE * 0.75 * scale_ratio)
    VERTICAL_SPACING = int(settings.FONT_SIZE * 0.5 * scale_ratio)
    TEXT_PADDING = int(60 * scale_ratio)
    TEXT_MARGIN = (int(15 * scale_ratio), int(15 * scale_ratio))
    ...
    try:
        ...
        # 360p -> 500k, 480p -> 1000k, 720p -> 2500k, 1080p -> 4500k
        bitrate_map = {
            360: "500k",
            480: "1000k",
            720: "2500k",
        ...
        # Default to 2500k if resolution not in map, though enum ensures it is.
        target_bitrate = bitrate_map.get(request.resolution, "2500k")
        logger.info(f"Using Bitrate: {target_bitrate} for {request.resolution}p")
